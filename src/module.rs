...
impl<'a> RootModule<'a> {
    pub fn new(source: RootModuleSource, toml_path: &'a str) -> Self {
        let toml_raw = fs::read_to_string(toml_path)
            .unwrap_or(fs::read_to_string(utils::find_workspace_root().join(toml_path))
                .unwrap_or(fs::read_to_string(PathBuf::from(
                    env::var("CARGO_MANIFEST_DIR").expect("Expected CARGO_MANIFEST_DIR to be in env")
                ).join(toml_path))
                    .unwrap_or_default()));
        let toml: Value = toml_raw.parse().unwrap_or_else(|_| {
            panic!("Failed to parse toml file: {}", toml_path)
        });
    ...
    pub fn build(self) -> Self {
        ...
        for pattern in &self.source.inclusion_pats {
            inclusions.add(Glob::new(&pattern.to_string()).expect("Expected a valid glob pat string"));
            println!("Added inclusion pattern: {}", pattern);
            literals.push(pattern.to_string());
        }
        for pattern in &self.source.exclusion_pats {
            exclusions.add(Glob::new(&pattern.to_string()).expect("Expected a valid glob pat string"));
            println!("Added exclusion pattern: {}", pattern);
            literals.push(format!("!{}", pattern.to_string()));
        }
        let fields = TomlFields::from(self.toml.clone())
            .with_inclusion_globs(Some(inclusions.build().expect("Expected a succesful glob set build")))
            .with_exclusion_globs(Some(exclusions.build().expect("Expected a succesful glob set build")))
