...
impl<'a> TomlFields<'a> {
    ...
    pub fn build(mut self) -> Self {
        ...
        self.extract_matched_paths_from_value(self.root_value.expect("Expected a root value when building TomlFields"), ROOT, 0);
    ...
    pub fn get_toml_children_of(&self, this_idx: usize) -> TomlFields<'a> {
        TomlFields::<'a> {
            fields: self.fields
                .iter()
                .filter(|field| { field.parent == Some(this_idx) && self.index_of(field).unwrap_or(usize::MAX) != this_idx })
                .cloned()
                .collect::<Vec<_>>(),
    ...
    pub fn get_relative_parent_of_field(&'a self, this_field: &'a TomlField<'a>) -> &'a TomlField<'a> {
        ...
        let relative_parent_field = self.get_by_name(&relative_parent_name)
            .unwrap_or_else(|| panic!("Expected a valid relative parent field ({} didn't exist, processing {})", &relative_parent_name, this_field.name));
    ...
    pub fn get_relative_children_of(&'a self, this_idx: usize) -> TomlFields<'a> {
        let this_field = self.get_field(this_idx).expect("Expected a valid field");
        let children = self.fields.iter().filter(|field| {
            let idx = self.index_of(field).expect("Expected a valid index of a field that exists");
            idx != this_idx && self.get_relative_parent_of_field(field) == this_field
        }).cloned().collect::<Vec<TomlField>>();
        TomlFields::<'a> {
            fields: children,
            patterns: self.patterns.clone(),
            root_value: self.root_value,
    ...
    pub fn extract_matched_paths_from_value(&mut self, value: &'a Value, _path: &str, parent_idx: usize) {
        ...
        let (mut field, field_idx) = if parent_idx == 0 && self.fields.is_empty() {
            ...
            let field = TomlField::new(
                path.split('.').last().expect("Expected a valid path to extract name from"),
        ...
        match value {
            ...
            _ => {
                ...
                let mut skip = !((path == ROOT) || ((self.patterns.inclusions.is_none() || self.patterns.inclusions.as_ref().expect("Expected inclusion globs").is_match(&path)) &&
                    (self.patterns.exclusions.is_none() || !self.patterns.exclusions.as_ref().expect("Expected exclusion globs").is_match(&path))));
                if is_alias && skip {
...
impl<'a> TomlFields<'a> {
    ...
    fn generate_module(&self, idx: usize, tokens: &mut TokenStream2) {
        ...
        let module_name = self.get_field(idx)
            .expect("Expected a valid index to an existing field")
            .path.split('.').last()
            .expect("Expected there to be at least one node from split by '.'");

        let mod_ident: Option<syn::Ident> = if !module_name.is_empty() { Some(format_ident!("{}", crate::utils::to_valid_ident(module_name).to_lowercase())) } else { None };
        ...
        for field in relative_children_fields_iter.iter().filter(|f| !f.is_table()) {
            let (ty, val) = crate::utils::convert_value_to_tokens(field.value);
            let const_name = format_ident!("{}", crate::utils::to_valid_ident(&field.name).to_uppercase());
            mod_tokens.extend(quote! { pub const #const_name: #ty = #val; });
        }

        // generate submodules for recursive hierarchy
        for submod in relative_children_fields_iter.iter().filter(|f| f.is_table()) {
            // println!("    >> Generating submodule {} for: {}", submod.name, module_name);
            self.generate_module(self.index_of(submod).expect("Expected a valid child that exists and thus has an index"), &mut mod_tokens);
        }
